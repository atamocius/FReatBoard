...
local transposeSelector = require('ui/main_page/transpose_selector')
...
function index.newMainPage()
  ...
  -- MARK: handleTransposeChange
  local function handleTransposeChange(
      selectedTransposeOffset
  )
    -- reaper.ShowConsoleMsg(selectedTransposeOffset .. '\n')
    -- reaper.ShowConsoleMsg('---------------------\n')

    self.selectedTransposeOffset = selectedTransposeOffset
  end
  ...
  local function handleFretboardClick(fret, string, note)
    if self.selectedModeIndex == 2 then
      ...
      insertNote(string, note.value + self.selectedTransposeOffset, self.selectedVelocityValue)
  ...
  local function handleSubmitClick()
    if self.selectedModeIndex == 1 then
      ...
      for i = 1, #notes do
        ...
        insertNote(n.string, n.note.value + self.selectedTransposeOffset, self.selectedVelocityValue)
      end
      -- reaper.ShowConsoleMsg('velocity: ' .. self.selectedVelocityValue .. '\n')
  ...
  local function render()
    ...
    transposeSel = transposeSelector.newTransposeSelector(
      642, 40, 11,
      self.selectedTransposeOffset,
